...
def browser_tool_usage_guidance() -> str:
    return (
        ...
        "Do not assume prior-turn sandbox limitations still apply. If these tools are exposed in this run, use them.\n"
        "If the user asks a quantitative question about attached data, compute the answer from the files instead of refusing. "
        "Start with files.list when needed, then use tables.preview, tables.profile, or python.execute.\n"
        ...
        "Friendly file references from files.list should be passed to read_table/read_text/file_info. "
        "If you use raw pandas or open(), first resolve the actual sandbox path with file_info(file_ref)['path'] "
        "instead of assuming the reference name is a relative filename.\n"
        "For structured Python answers, assign the value to result or leave it as the final bare expression. "
        "python.execute returns structured output shaped like {'stdout': str, 'result': json, 'files': {...}}.\n"
        "Example mean pattern: df = read_table('data.csv'); s = pd.to_numeric(df['Electric Range'], errors='coerce'); "
        "result = {'mean': float(s.dropna().mean()), 'count_non_null': int(s.notna().sum())}.\n"
        "Prefer python.execute for direct calculations over attached datasets such as averages, deltas, percent changes, regressions, forecasts, rankings, and aggregations.\n"
        "When files.list returns a reference_name, prefer that friendly reference over long opaque ids.\n"
        "If a term is mildly ambiguous but a standard default exists, make the default assumption, state it briefly, and continue. "
        "For annual series, 'delta as a %' usually means year-over-year percent change.\n"
